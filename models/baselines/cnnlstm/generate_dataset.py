...
class Workspace():
    ...
    def real_data(self, root_dir):
        runs = natsorted(glob.glob(root_dir+"*/"))
        n_bufs = 38            # Start here because we have already processed 37

        for i, run in enumerate(runs):
            if int(run.split("/")[-2]) < 41:   # Skip the first 40 runs because they are processed
                continue
            ...
            print("We have ", len(buffer["obs"]), " samples")
            if self.use_audio:
                stringg = 'audio'
            else:
                stringg = 'video'
            ...
            write_path = f"/home/punygod_admin/SoundSense/soundsense/data/cnn_baseline_data/seventysix/" 
            n_bufs += 1
            self.buffer = []
            if len(buffer["obs"]) == 0:
                continue
...
def main():
    import os
    os.makedirs('/home/punygod_admin/SoundSense/soundsense/data/cnn_baseline_data/seventysix/actions/', exist_ok=True)
    os.makedirs('/home/punygod_admin/SoundSense/soundsense/data/cnn_baseline_data/seventysix/audio/', exist_ok=True)
    os.makedirs('/home/punygod_admin/SoundSense/soundsense/data/cnn_baseline_data/seventysix/video/', exist_ok=True)
    os.makedirs('/home/punygod_admin/SoundSense/soundsense/data/cnn_baseline_data/seventysix/metadata/', exist_ok=True)
