...
class Encoder(nn.Module):
    ...
    def __init__(self, obs_shape, feature_dim, num_layers, num_filters, output_dim, output_logits):
        super().__init__()
...
class Actor(nn.Module):
    ...
    def forward(self, obs, detach_encoder = False, squashed = True):
...
class DRQAgent(object):
    ...
    def __init__(self, obs_shape, action_shape, action_range, device,
        ...
        # self.actor.encoder.copy_conv_weights_from(self.critic.encoder)    # TODO
    ...
    def act(self, lowdim, obs, sample=False, squash = True, base_action = None, batch = False):
        ...
        if base_action is not None: #residual
            ...
            dist = self.actor(obs, base_action, squashed = squash) #happens only when we are in residual mode; weird polymorphism
        else:
            dist = self.actor(obs, squashed = squash)
    ...
    def to_tensor(self, obs, action, reward = None, done_no_max = None):
        ...
        return obs, action, reward, done_no_max
...
#         input(new_obs.shape)
    ...
    def update_bc(self, replay_buffer, logger, step, squash = True):
        ...
        t3 = time.time()
        # Get lowdim, obs, action from next(replay_buffer)
